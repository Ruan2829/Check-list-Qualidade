...
class PDF(FPDF):
    def header(self):
        ...
        self.image("temp_logo.jpeg", 10, 8, 33)  # Usar o logo gerado
...
def gerar_pdf_multifotos(inspecao_data):
    ...
    for sistema, detalhes in inspecao_data["sistemas"].items():
        ...
        for i, foto_path in enumerate(detalhes['fotos'][:4]):  # Limita a 4 fotos
            try:
...
def estrelas_clicaveis(label, max_stars=5, default_stars=3, key=None):
...
for sistema in sistemas:
    ...
    estrelas = estrelas_clicaveis(f"Avalie o sistema {sistema}", key=sistema)
